...
import type { TransformedPaper } from "@/lib/types";
...
interface PaperClientProps {
  initialPaper: TransformedPaper;
}

export function PaperClient({ initialPaper }: PaperClientProps) {
  const router = useRouter();
  const [paper, setPaper] = useState<TransformedPaper>(initialPaper);
  ...
  const triggerRegeneration = async (notes: string) => {
    ...
    setPaper((prev) => ({ ...prev, status: "in_progress" }));
  ...
  const sourceFiles = paper.files.map((f) => ({
    name: f.name,
    type: f.mimeType,
    size: f.size,
  }));
